...
**工程结论**：变量的内存区域（栈/堆/全局）由声明方式和位置决定，不是地址值决定。
...
**工程结论**：能用引用传参就不用指针，能少一次判空就少一次出错机会。
...
**工程结论**：操作连续内存时，用指针游走比下标计算更贴近硬件，也更高效。
...
**工程结论**：永远不要假设两次 `new` 返回的地址有任何固定关系。需要连续内存请用数组 `new T[N]` 或 `std::vector`。
